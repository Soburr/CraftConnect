            ...
            @else
                <div id="recommendedGrid" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @foreach ($recommendedArtisans as $index => $artisan)
                        <div
                            class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 text-center flex flex-col h-full hover:shadow-md transition {{ $index >= 3 ? 'hidden extra-artisan' : '' }}">
                            <!-- Artisan Avatar -->
                            ...
                            @if ($index === 0)
                                <span
                                    class="inline-block px-2 py-1 text-xs font-semibold text-white bg-yellow-500 rounded-full mb-2">
                                    🏆 Top Rated
                                </span>
                            @else
                            ...
                            <!-- Tier Badge -->
                            <span
                                class="inline-block px-2 py-1 text-xs font-medium rounded-full mb-3
                        ...
                        @else bg-orange-100 text-orange-700 @endif">
                            ...
                            <form method="POST" action="{{ route('client.book-artisan', $artisan->id) }}" class="mt-auto">
                                ...
                                <button type="submit"
                                    class="w-full px-4 py-2 text-sm bg-green-600 text-white rounded hover:bg-green-700 transition">
                                    Book Now
                                </button>
                            </form>
                        </div>
